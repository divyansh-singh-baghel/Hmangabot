...
# FIX: Ab DATABASE_CHANNEL string (jaise @channelname) aur Number dono accept karega
...
async def build_and_send_premium_post(title, tags, pages_count, cover_url, pdf_files, dm_chat_id=None, is_autopost=False):
    ...
    # FIX: No fallback to direct reply! If db upload fails, show error and stop.
    if not db_msg_ids:
        if dm_chat_id:
        ...
        # File delete karke wapas return kar jao
        for pdf_file in pdf_files:
    ...
    # FIX: Removed the "Generated by @DSB_07" line
    post_caption = (
        f"📖 **Name:** `{title}`\n\n"
        f"🏷 **Tags:** `{', '.join(tags)}`\n"
        f"📄 **Pages:** `{pages_count}`"
    ...
    # FIX: Send to DM AND Channel if AUTO_POST is set
    if not is_autopost:
